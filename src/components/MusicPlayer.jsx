...
export default function MusicPlayer({ isLightMode }) {
  ...
  return (
    <div className={`rounded-[32px] p-4 flex items-center gap-4 mb-8 relative shadow-lg overflow-hidden border transition-colors duration-700 ${isLightMode ? 'bg-white border-white shadow-pink-100/50 border-l-[6px] border-l-[#f472b6]' : 'bg-[#151518] border-white/[0.02]'}`}>
      ...
      {/* Vinyl CD Component */}
      <div 
        className={`w-[85px] h-[85px] ml-1 shrink-0 rounded-full bg-[#111] border-[4px] border-[#222] shadow-xl flex items-center justify-center relative overflow-hidden animate-[spin_4s_linear_infinite]`}
      ...
      <div className="flex-1 py-1 ml-2">
        <h3 className={`text-[17px] font-bold mb-0.5 tracking-tight transition-colors duration-500 ${isLightMode ? 'text-slate-800' : 'text-white'}`}>뭐해 '라는 씨앗에서 (Official Audio)</h3>
      ...
      <button onClick={togglePlay} className={`w-[48px] h-[48px] rounded-full flex items-center justify-center mr-4 shrink-0 hover:scale-105 transition ${isLightMode ? 'bg-white text-black border-2 border-[#e2e8f0] shadow-[0_0_20px_rgba(226,232,240,0.6)]' : 'bg-white text-black hover:bg-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.05)]'}`}>
        {isPlaying ? (
